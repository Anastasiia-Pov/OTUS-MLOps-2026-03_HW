...
S3_DP_LOGS_BUCKET = S3_BUCKET_NAME                              # Путь для логов Data Proc
...
with DAG(
    ...
    train_model = DataprocCreatePysparkJobOperator(
        ...
        dag=dag,
        args=[
            # Pass the dataset root. train.py discovers Parquet files
            # recursively inside the date-named directories.
            "--input", S3_INPUT_DATA_BUCKET,
            # Pass a base path. train.py adds a runtime UTC timestamp.
            "--output", S3_OUTPUT_MODEL_BUCKET,
            "--sample-size", "1000000",
