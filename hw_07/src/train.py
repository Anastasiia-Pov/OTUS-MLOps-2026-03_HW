...
def list_parquet_directories(spark, input_path):
    """List immediate ``*.parquet`` directories without reading their files."""
    ...
    directory_statuses = filesystem.listStatus(hadoop_path)
    ...
    return sorted(parquet_directories)
...
def load_data(
    ...
    sample_size : int
        Maximum number of randomly selected records
    sample_fraction : float
        Fraction used to build a random candidate sample before applying the limit
    sample_seed : int, optional
        Random seed. A new seed is generated when it is not provided.
    use_all_files : bool
        Read every Parquet directory instead of choosing one random directory.

    Returns
    -------
    tuple
        (train_df, test_df, selected_input_directory) - prepared DataFrames and
        the selected directory path, or None in all-files mode
    """
    print(f"DEBUG: Начинаем загрузку данных из: {input_path}")
    try:
        if sample_size <= 0:
            raise ValueError("sample_size должен быть больше нуля")
        ...
        if use_all_files:
            print(f"DEBUG: Рекурсивное чтение Snappy Parquet файлов из {input_path}")
            df = (
                spark.read
            ...
            parquet_directories = list_parquet_directories(spark, input_path)
            if not parquet_directories:
                raise FileNotFoundError(
                    f"В {input_path} не найдены директории *.parquet"
                )

            selected_input_directory = (
                parquet_directories[sample_seed % len(parquet_directories)]
            )
            print(
                "DEBUG: Случайно выбрана Parquet директория: "
                f"{selected_input_directory} "
                f"(всего директорий: {len(parquet_directories)})"
            )
            df = (
                spark.read
                .option("recursiveFileLookup", "true")
        ...
        # Print the schema without triggering a full-data Spark action.
        print("Dataset Schema:")
        df.printSchema()

        print(
            "DEBUG: Создаем случайную выборку: "
            f"sample_size={sample_size}, "
            f"sample_seed={sample_seed}"
        )

        if use_all_files:
            # Sampling first reduces the random sort from the complete dataset
            # to a small candidate set.
            candidate_df = df.sample(
                withReplacement=False,
                fraction=sample_fraction,
                seed=sample_seed,
            )
        else:
            # Only the selected date directory is scanned. Random ordering
            # produces a row sample across all Parquet files inside it.
            candidate_df = df

        sampled_df = (
            candidate_df
            .orderBy(F.rand(sample_seed + 1))
            .limit(sample_size)
            .persist(StorageLevel.MEMORY_AND_DISK)
        )

        actual_sample_size = sampled_df.count()
        print(f"DEBUG: Размер случайной выборки: {actual_sample_size}")
        if actual_sample_size < sample_size:
            if use_all_files:
                print(
                    "WARNING: Получено меньше строк, чем запрошено. "
                    "Увеличьте --sample-fraction."
                )
            else:
                print(
                    "WARNING: Выбранная директория содержит меньше строк, "
                    "чем --sample-size."
                )
        ...
        return train_df, test_df, selected_input_directory
    except Exception as e:
...
EXCLUDED_COLS = {
    ...
    "terminal_id",         # categorical identifier
    "tx_fraud",            # target
    "tx_fraud_scenario",   # target leakage
  }
...
def prepare_features(train_df, test_df):
    """Select numeric features and remove rows with missing values."""
    ...
    try:
        ...
        feature_cols = [
            ...
            and is_numeric(dtypes[column])
        ]

        if not feature_cols:
            raise ValueError(
                "После исключения служебных столбцов "
                "не осталось числовых признаков"
            )

        if LABEL_COL not in train_df.columns:
            raise ValueError(
                f"Целевая переменная '{LABEL_COL}' отсутствует"
            )
        ...
        if missing_test_cols:
            raise ValueError(
                f"В test_df отсутствуют столбцы: {missing_test_cols}"
            )

        print(
            f"DEBUG: Выбрано {len(feature_cols)} признаков: "
            f"{feature_cols}"
        )

        required_cols = feature_cols + [LABEL_COL]

        # Операция остаётся ленивой до начала обучения.
        ...
        print(f"ERROR: Ошибка подготовки признаков: {error}")
        print(f"Traceback: {traceback.format_exc()}")
        raise
...
def train_model(
    ...
    feature_cols,
    model_type="rf",
    run_name="fraud_detection_model",
    sample_size=None,
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            if sample_seed is not None:
                mlflow.log_param("sample_seed", sample_seed)
            if selected_input_directory is not None:
                mlflow.log_param(
                    "selected_input_directory",
                    selected_input_directory,
                )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1_000_000,
        help="Maximum number of randomly selected input records",
    )
    parser.add_argument(
        "--sample-fraction",
        type=float,
        default=0.001,
        help="Candidate sampling fraction used only with --use-all-files",
    )
    parser.add_argument(
        "--sample-seed",
        type=int,
        default=None,
        help="Optional reproducible sampling seed; random by default",
    )
    parser.add_argument(
        "--use-all-files",
        action="store_true",
        help="Sample from all directories instead of one random Parquet directory",
    )
    ...
    output_path = f"{args.output.rstrip('/')}/model_{run_timestamp}"
    run_name = args.run_name or f"training_{run_timestamp}"
    print(
        "DEBUG: Аргументы командной строки:",
        {
            ...
            "s3_access_key": "***" if args.s3_access_key else None,
            "s3_secret_key": "***" if args.s3_secret_key else None,
        },
    )
    ...
    try:
        ...
        train_df, test_df, selected_input_directory = load_data(
            spark,
            args.input,
            sample_size=args.sample_size,
            sample_fraction=args.sample_fraction,
        ...
        model, metrics = train_model(
            ...
            run_name,
            sample_size=args.sample_size,
            sample_fraction=(args.sample_fraction if args.use_all_files else None),
            sample_seed=sample_seed,
            selected_input_directory=selected_input_directory,
        )
