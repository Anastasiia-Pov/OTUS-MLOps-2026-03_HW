locals {
  existing_bucket_name = "otus-bucket-b1g4ki09n8igs1si54v2"
}
...
# Storage ресурсы - bucket уже существует в YC - поэтому вмонтируем его 
...
resource "yandex_compute_instance" "proxy" {
  ...
  metadata = {
    ...
    user-data = templatefile("${path.root}/scripts/user_data.sh", {
      ...
      access_key                  = yandex_iam_service_account_static_access_key.sa-static-key.access_key
      secret_key                  = yandex_iam_service_account_static_access_key.sa-static-key.secret_key
      s3_bucket                   = local.existing_bucket_name
