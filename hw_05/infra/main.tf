...
resource "null_resource" "update_env" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      sed -i.bak "s|^AIRFLOW_URL=.*|AIRFLOW_URL=https://c-$AIRFLOW_ID.airflow.yandexcloud.net|" "$ENV_FILE"
      sed -i.bak "s|^AIRFLOW_ADMIN_PASSWORD=.*|AIRFLOW_ADMIN_PASSWORD=$AIRFLOW_ADMIN_PASSWORD|" "$ENV_FILE"
      sed -i.bak "s|^S3_ENDPOINT_URL=.*|S3_ENDPOINT_URL=$STORAGE_ENDPOINT_URL|" "$ENV_FILE"
      sed -i.bak "s|^S3_AIRFLOW_BUCKET=.*|S3_AIRFLOW_BUCKET=$BUCKET_NAME|" "$ENV_FILE"
      sed -i.bak "s|^S3_ACCESS_KEY=.*|S3_ACCESS_KEY=$ACCESS_KEY|" "$ENV_FILE"
      sed -i.bak "s|^S3_SECRET_KEY=.*|S3_SECRET_KEY=$SECRET_KEY|" "$ENV_FILE"
      rm -f "$ENV_FILE.bak"
    EOT
  }
